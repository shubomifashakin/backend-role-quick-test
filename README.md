...
- **Caching**: We can introduce a redis backed cache layer for read heavy routes, (like the transaction history one). Transaction details barely change after the transaction is complete. To speed up reduce response times, we can cache the details in a kv store like redis and reduce requests to our actual database.
...
Env is committed only because its need for easy setups.
